...
We start by combining the corresponding ids with the example data
...
rl_data = cbind(RLdata10000, id = identity.RLdata10000)

# remove for memory
rm(RLdata10000)
rm(identity.RLdata10000)
...
With our reformatted data in hand, we can now proceed with the experiment. Since there are 10,000 names in the example data set, our experiment will require comparing each of the names with every name. To avoid running such a computationally expensive task multiple times, we will run the experiment and save our results in RDS files. Below is the path for the directory that will store the results and the paths for the results files.
...
Now we will run the experiment below and save our results
...
# results dataframe
results = data.frame(
  # make responses a factor for PR curve functionality
...
Next, we combine the PR curve dataframes from each metric to plot all curves in a single figure. We also gather the AUCs from each metric.
...
Now our single `curve_df` dataframe contains the PR curve dataframes from each metric with the additional `Metric` column indicating which metric each row is associated with
...
We see the PR AUCs in the other list
